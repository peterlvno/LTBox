...
**`4. 시스템 업데이트 재활성화`**
비활성화된 자동 업데이트 설정을 원래대로 복원하고 시스템 업데이트 패키지를 재설치합니다.

**`5. 시스템 업데이트 후 부팅 실패 복구`**
변환된 펌웨어를 설치한 기기에서 실수로 전체 OTA 업데이트를 진행하여 부팅 문제가 발생했을 때, 필수 파티션만을 패치하여 복구를 시도합니다.
...
**`r. 기기 재부팅`**
기기를 시스템, Fastboot, Fastbootd 또는 EDL 모드로 재부팅합니다.
...
* **롤백 인덱스 변경:** 안티 롤백(Anti-Rollback) 패치 관련 기능을 ON/OFF합니다.
* **ADB 작업 건너뛰기:** ADB 연결 확인 과정을 ON/OFF합니다. 기기가 이미 EDL이나 fastboot 모드일 때 유용합니다.
